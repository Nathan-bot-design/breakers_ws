...
DiffDriveArduino::DiffDriveArduino()
    : logger_(rclcpp::get_logger("DiffDriveArduino")),
      imu_orientation_x_(0.0), imu_orientation_y_(0.0),
      imu_orientation_z_(0.0), imu_orientation_w_(1.0),
      imu_angular_vel_x_(0.0), imu_angular_vel_y_(0.0), imu_angular_vel_z_(0.0),
      imu_linear_acc_x_(0.0), imu_linear_acc_y_(0.0), imu_linear_acc_z_(0.0)
{}
...
std::vector<hardware_interface::StateInterface> DiffDriveArduino::export_state_interfaces()
{
  ...
  // IMU states (ROS 2 standard naming)
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "orientation.x", &imu_orientation_x_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "orientation.y", &imu_orientation_y_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "orientation.z", &imu_orientation_z_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "orientation.w", &imu_orientation_w_));

  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "angular_velocity.x", &imu_angular_vel_x_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "angular_velocity.y", &imu_angular_vel_y_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "angular_velocity.z", &imu_angular_vel_z_));

  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "linear_acceleration.x", &imu_linear_acc_x_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "linear_acceleration.y", &imu_linear_acc_y_));
  state_interfaces.emplace_back(
    hardware_interface::StateInterface("imu", "linear_acceleration.z", &imu_linear_acc_z_));
...
return_type DiffDriveArduino::read(const rclcpp::Time & /* time */, const rclcpp::Duration & /* period */)
{
  ...
  // --- IMU ---
  double yaw, dyaw, ax, wz;
  arduino_.readImuValues(yaw, dyaw, ax, wz);

  // Convert yaw (assuming roll=pitch=0) to quaternion
  imu_orientation_x_ = 0.0;
  imu_orientation_y_ = 0.0;
  imu_orientation_z_ = std::sin(yaw * 0.5);
  imu_orientation_w_ = std::cos(yaw * 0.5);

  // Angular velocity
  imu_angular_vel_x_ = 0.0;
  imu_angular_vel_y_ = wz;     // depends on your IMU axis mapping
  imu_angular_vel_z_ = dyaw;   // yaw rate

  // Linear acceleration
  imu_linear_acc_x_ = ax;
